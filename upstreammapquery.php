...
if($geoages!=""){
	$geoagess=explode(",",$geoages);
	$geonum=0;
	foreach($geoagess as $geoage){
...
if($_GET['sample_pkey']!=""){
	...
	}elseif($myrow->ecproject=="fissiontrack"){
		$showdetritalmethod="Fission Track";
	}
...
?>
	...
	<table class="aboutpage" width="250" cellpadding="2" cellspacing="1" bgcolor="#333333">
  	<tr>
		...
		<td><div align="right"><a href="javascript:show_my_form(<?=$mylat?>,<?=$mylon?>,<?=$pkey?>,<?=$myzoom?>,'<?=$agemin?>','<?=$agemax?>','<?=$detritaltype?>','<?=$detritalmineral?>','<?=$detritalmethod?>','<?=$geoages?>','<?=$showall?>');" ><font color="#FFFFFF"><strong>back</strong></font></a></div></td>
		<td>&nbsp;</td>
		</tr>
		</table>
		</td>
...
<tr><td colspan="2"><div align="center"><a style="color:#3333CC;font-size:8pt;font-weight:bold; " href="javascript:popwindow('upstreamviewfile.php?pkey=<?=$myrow->sample_pkey?>');"><img src="/images/magglass.png" border="0"> VIEW SAMPLE DETAILS</a></div></td></tr>
<?
...
<?
	...
	$mapstring="select sample.sample_pkey as pkey, 
							...
							youngest_frac_date,
							age_min, age_max, age_value, one_sigma, age_name,
							getagetypes(sample.sample_pkey) as agetypes,filename
							from sample 
							left join sample_age on sample.sample_pkey = sample_age.sample_pkey
							left join groupsamplerelate on sample.sample_pkey = groupsamplerelate.sample_pkey
							left join grouprelate on groupsamplerelate.group_pkey = grouprelate.group_pkey
							left join groups on grouprelate.group_pkey = groups.group_pkey
							where (sample.publ=1 or sample.userpkey=$userpkey or ((grouprelate.users_pkey=$userpkey and grouprelate.confirmed=true)) or groups.users_pkey=$userpkey)
...
if($conc=="yes"){
	...
	//ST_Contains(('Polygon((-124.31875 44.2875,-115.0375 46.3125,-114.75625 45.4125,-124.43125 42.99375,-124.31875 44.2875))'),loc.mypoint)
	...
	$mapstring.=" and st_GeomFromText('Polygon(($coordbox))') ~ mypoint ";
	...
	//echo "mapstring: ".nl2br($mapstring);
	...
	/*
	$myrow=$db->get_row("select sample.sample_pkey as pkey,
						sample_id,
						longitude,
						latitude, 
						age_min,
...
?>
			...
			<tr>
				...
				<table class="aboutpage" width="100%">
				<tr>
				<td><font color="#FFFFFF"><strong>Sample Details: <?=$asample_pkey?></strong></font></td>
				<td>&nbsp;</td>
				<td>&nbsp;</td>
				</tr>
				</table>
			...
			<?}?>
      		<table class="aboutpage" width="250" cellspacing="1" cellpadding="2">
				...
				<tr>
          		...
          		<td><a href="javascript:show_detail(<?=$myrow->pkey?>,<?=$mylat?>,<?=$mylon?>,<?=$pkey?>,<?=$myzoom?>,'<?=$agemin?>','<?=$agemax?>','<?=$detritaltype?>','<?=$detritalmineral?>','<?=$detritalmethod?>','<?=$geoages?>','<?=$showall?>');" >detail</a></td>
        		</tr>
        		<? } //end for each row ?>
