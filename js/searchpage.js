...
function updatecoordinates(pointstring){
	...
	document.getElementById('mapinfo').innerHTML='<a onclick="showmap();" ><img src="http://www.geochron.org/ajaxpoly.php?coords='+pointstring+'"></a><br><div align="center">Polygon Set</div>\
...
function doararcheck(){
	...
	if(document.getElementById('ararcheck').checked){
		//alert('checked');
		Shadowbox.open({
			content:    'agetypeselector.php?project=arar;ararxls',
			player:     "iframe",
...
function dosearch() {
	...
	if(north!="" && east!="" && south!="" && west!="" && coordinates!=""){
		...
		if(confirm("This will clear your polygon setting. Are you sure?")){
			...
			document.getElementById('mapinfo').innerHTML='<a onclick="showmap();" ><img src="http://www.geochron.org/ajaxpoly.php?coords='+ coordstring +'"></a></a><br>Use Geochron Dynamic Mapper   ';
