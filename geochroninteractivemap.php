...
$mapstring="select longitude,
latitude from ($querystring) bar
";

/*
echo nl2br("select min(latitude) as south,
...
<script src="js/dynmapajax.js" type="text/javascript"></script>
...
<tr><td>
	...
	<table class="aboutpage" width="250" cellpadding="2" cellspacing="1" bgcolor="#333333">
		...
		<br><hr><br>
		<div id="allcount" align="center" style="padding-top:20px;">Search Results Count: <?=$totalcount?></div>
    ...
    <script type="text/javascript">
		...
		*/
		...
		topo.addOptions({isBaseLayer: true});
		    ...
		    "dynmap.php?pkey=<?=$pkey?>&label=yes&zoom="+myzoom+"&", {layers: 'basic'},
                { singleTile: true, ratio: 1 } );
			...
			map.events.register("moveend", map, function(e) {
          	...
          	map.events.register("click", map, function(e) {
				...
				show_my_form(lonlat.lat, lonlat.lon, <?=$pkey?>, myzoom, showfig);
		...
		function drawagainb() {
		    ...
		    "dynmap.php?pkey=<?=$pkey?>&", {layers: 'basic'},
                { singleTile: true, ratio: 1 } );
            foolayer.addOptions({isBaseLayer: false});
		...
		function dosearch() {
			...
			myrandnum=Math.floor(Math.random()*11);
            foolayer = new OpenLayers.Layer.WMS.Untiled( "Samples",
		    "dynmap.php?pkey=<?=$pkey?>&randnum="+unixtime+myrandnum+"&", {layers: 'basic'},
                { singleTile: true, ratio: 1 } );
            foolayer.addOptions({isBaseLayer: false});
			map.addLayers([foolayer]);
			return false;
...
<div id="mycount"></div>
		...
		getcount = function () {
			...
			urlstring="dynmap.php?pkey=<?=$pkey?>&randnum="+unixtime+myrandnum+"&zoom="+myzoom
