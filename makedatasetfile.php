...
if($_GET['jasondatasetpkey']!=""){
	$dataset_pkey=$_GET['jasondatasetpkey'];
}
...
if($dataset_pkey!=""){
	...
	$dcount=$db->get_var("select count(*) from datasetrelate where dataset_pkey = $dataset_pkey");
	
	if($dcount>0){
	
		
	
		$dataset_row=$db->get_row("select * from datasets where dataset_pkey=$dataset_pkey");
		
		$datasetname=$dataset_row->datasetname;
		
		$linkstring=$dataset_row->linkstring;
		
		mkdir("publishtemp/$linkstring");
		
		$xlsfilename="publishtemp/$linkstring/data.xls";
		
		$dirname="publishtemp/$linkstring/";
		...
		//echo "publish/$linkstring.zip";
	
	
		//create zip archive
		$z = new ZipArchive();
		$z->open("publish/$linkstring.zip", ZIPARCHIVE::CREATE);
		$z->addEmptyDir("geochrondataset_$linkstring");
		...
		$querystring="select 
			...
			$rows=$db->get_results($querystring);
						...
						// Create an instance
						$xls =& new Spreadsheet_Excel_Writer($xlsfilename);
					...
					$columnnames=array("Sample ID","Unique ID       ","Metadata URL    ","Sample Description","Sample Comment","Longitude","Latitude","Method","Material","Age (Ma)",chr(177)." 2s (abs)","Age Type","GeoObject Type","GeoObject Class","Collection Method","Analyst Name","Laboratory Name","Collector","Rock Type","Primary Location Name","Primary Location Type","Location Description","Locality","Locality Description","Country","Province");
					...
					if(count($rows)>0){
						...
						foreach($rows as $row){
							...
							}elseif($row->ecproject=="squid"){
								$detritalmethod="U-Pb";
							}elseif($row->ecproject=="zips"){
								$detritalmethod="U-Pb";
							}
							...
							if($row->ecproject=="squid"){
								$method="SQUID";
							}
							if($row->ecproject=="zips"){
								$method="ZIPS";
							}
							...
							if(file_exists("files/$thisfile")){

								$z->addFile("files/$thisfile", "geochrondataset_$linkstring/$thisfile");
							...
							//echo "<a href=\"files/$filename\" target=\"_blank\">files/$filename</a><br>";
							...
							if($dom->Load($xmlfile)){
								...
								if($ecproject=="redux"){
									...
									/*
									  <sampleDateModels>
										<SampleDateModel>
										  <name>weighted mean 206Pb/238U (Th-corrected)</name>
										  <value>201498410.480246126651763916015625</value>
										  <uncertaintyType>ABS</uncertaintyType>
										  <oneSigma>16341.99808343872791738249361515045166015625</oneSigma>
										  <meanSquaredWeightedDeviation>0.43523956684226139390858634214964695274829864501953125</meanSquaredWeightedDeviation>
										  <internalTwoSigmaUnct>32683.9961668774558347649872303009033203125</internalTwoSigmaUnct>
										  <internalTwoSigmaUnctWithTracerCalibrationUnct>62153.28495745817781426012516021728515625</internalTwoSigmaUnctWithTracerCalibrationUnct>
										  <internalTwoSigmaUnctWithTracerCalibrationAndDecayConstantUnct>224403.36801679941709153354167938232421875</internalTwoSigmaUnctWithTracerCalibrationAndDecayConstantUnct>
										  <includedFractionsVector>
											<fractionID>z1</fractionID>
											<fractionID>z2</fractionID>
											<fractionID>z4</fractionID>
											<fractionID>z5</fractionID>
											<fractionID>z7</fractionID>
										  </includedFractionsVector>
...
*/
									...
									*/
									...
									$sheet->write($y,8,"147Sm (ppm)",$formatfrachead);
									//$sheet->write($y,9,"[U]e",$formatfrachead);
									$sheet->write($y,9,"Th/U",$formatfrachead);
									$sheet->write($y,10,"He (nmol/g)",$formatfrachead);
									$sheet->write($y,11,"Mass (ug)",$formatfrachead);
									$sheet->write($y,12,"Ft",$formatfrachead);
									//$sheet->write($y,14,"Mean ESR",$formatfrachead);
									...
									foreach($fractions as $fraction){
										...
										$sheet->write($y,9,"$mythUu",$formatfrac);
										$sheet->write($y,10,"$myhe",$formatfrac);
										$sheet->write($y,11,"$mymass_ug",$formatfrac);
										$sheet->write($y,12,"$myft",$formatfrac);
								...
								//echo "failed: $row->sample_id $filename<br>";
								$y++;
								$y++;
							}
		...
		//OK, let's build a new tab for ECL info.
		$eclsheet =& $xls->addWorksheet('ECL Info');
		$eclsheet->write(0,0,"Information Needed by EarthChem Library",$formathead);
		
		$eclsheet->write(6,0,"SAMPLE NAME",$formatwhiteblue);
		$eclsheet->write(6,1,"IGSN",$formatwhiteblue);
		$eclsheet->write(6,2,"LATITUDE",$formatwhiteblue);
		$eclsheet->write(6,3,"LONGITUDE",$formatwhiteblue);

		
		$ecly=7;
		
		$eclrows=$db->get_results($querystring);
		
		foreach($eclrows as $row){
			$eclsheet->write($ecly,0,$row->sample_id,$formatinstr);
						
			$row->igsn=str_replace("SSR.","",$row->igsn);
			$row->igsn=str_replace("GCH.","",$row->igsn);
			
			$eclsheet->write($ecly,1,$row->igsn,$formatinstr);
			$eclsheet->write($ecly,2,$row->latitude,$formatinstr);
			$eclsheet->write($ecly,3,$row->longitude,$formatinstr);
			$ecly++;
		}
		...
		$xls->close(); 
			...
			//add xls file to zip archive
			$z->addFile("$xlsfilename", "geochrondataset_$linkstring/data.xls");
