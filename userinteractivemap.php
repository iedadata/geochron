    ...
    <script type="text/javascript">
		...
		*/
		...
		topo.addOptions({isBaseLayer: true});
		    ...
		    "userdynmap.php?pkey=<?=$pkey?>&label=yes&zoom="+myzoom+"&", {layers: 'basic'},
                { singleTile: true, ratio: 1 } );
		...
		function drawagainb() {
		    ...
		    "userdynmap.php?pkey=<?=$pkey?>&", {layers: 'basic'},
                { singleTile: true, ratio: 1 } );
            foolayer.addOptions({isBaseLayer: false});
		...
		function dosearch() {
			...
			myrandnum=Math.floor(Math.random()*11);
            foolayer = new OpenLayers.Layer.WMS.Untiled( "Samples",
		    "userdynmap.php?pkey=<?=$pkey?>&randnum="+unixtime+myrandnum+"&", {layers: 'basic'},
                { singleTile: true, ratio: 1 } );
            foolayer.addOptions({isBaseLayer: false});
			map.addLayers([foolayer]);
			return false;
...
<div id="mycount"></div>
		...
		getcount = function () {
			...
			urlstring="userdynmap.php?pkey=<?=$pkey?>&randnum="+unixtime+myrandnum+"&zoom="+myzoom
